  ...
  if (!is.na(rf_nodesize) &
    ...
    if (mode == "all") {
      ...
      if (length(rid) > 1) {
        ...
        set.seed(seed)
        fit <- gbm(
          formula = Surv(OS.time, OS) ~ ., data = est_dd2, distribution = "coxph",
          n.trees = 10000,
          interaction.depth = 3,
          n.minobsinnode = 10,
          shrinkage = 0.001,
      ...
      if (T) {
        ...
        if (length(rid) > 1) {
          ...
          set.seed(seed)
          cv.plsRcox.res <- cv.plsRcox(list(x = est_dd2[, rid], time = est_dd2$OS.time, status = est_dd2$OS), nt = 10, verbose = FALSE)
          fit <- plsRcox(est_dd2[, rid],
            time = est_dd2$OS.time,
      ...
      if (length(rid) > 1) {
        ...
        data <- list(
          x = t(est_dd2[, -c(1, 2)]), y = est_dd2$OS.time, censoring.status = est_dd2$OS,
          featurenames = colnames(est_dd2)[-c(1, 2)]
        )
        set.seed(seed)
        fit <- superpc.train(data = data, type = "survival", s0.perc = 0.5) # default
      ...
      set.seed(seed)
      fit <- gbm(
        formula = Surv(OS.time, OS) ~ ., data = est_dd, distribution = "coxph",
        n.trees = 10000,
        interaction.depth = 3,
        n.minobsinnode = 10,
        shrinkage = 0.001,
